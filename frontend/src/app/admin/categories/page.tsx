    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900">Category Taxonomy</h1>
          ...
          <button
            ...
            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 font-bold text-white text-xs rounded-xl shadow-md flex items-center gap-1.5 transition-all self-start sm:self-auto"
          >
            <PlusCircle className="w-4 h-4" /> Add Category
          </button>
        </div>

        {/* Categories List (White & Green Theme) */}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-emerald-50">
                {isLoading ? (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-400">Loading categories...</td>
                  </tr>
                ) : categories.length > 0 ? (
                  categories.map((c) => (
                    <tr key={c.id} className="hover:bg-emerald-50/30 transition-colors">
                      <td className="p-4 font-bold text-slate-900 flex items-center gap-2">
                        <Layers className="w-4 h-4 text-emerald-600" /> {c.name}
                      </td>
                      <td className="p-4 font-mono text-emerald-700">/{c.slug}</td>
                      <td className="p-4 text-slate-500">{c.description || "No description provided."}</td>
                      <td className="p-4">
                        <span className="px-2.5 py-0.5 font-bold rounded-full bg-emerald-100 text-emerald-800 text-[10px]">
                          ACTIVE
                        </span>
                      </td>
                  ...
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-400">No categories created yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full border border-emerald-100 shadow-2xl space-y-6 text-slate-900 animate-scale-in">
              <div className="flex items-center justify-between border-b border-emerald-50 pb-3">
                <h3 className="font-bold text-base flex items-center gap-2 text-slate-900">
                  <FolderTree className="w-5 h-5 text-emerald-600" /> Add Product Category
                </h3>
                <button
              ...
              <form onSubmit={handleCreateCategory} className="space-y-4 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                  />
                </div>
