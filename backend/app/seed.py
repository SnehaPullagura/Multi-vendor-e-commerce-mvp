...
async def seed_data():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Product 5: Quantum Horizon Smartwatch V4 (TechHub)
        p5 = Product(
            vendor_id=tech_vendor.id,
            category_id=sub_phones.id,
            title="Quantum Horizon Titanium Smartwatch V4",
            slug="quantum-horizon-titanium-smartwatch-v4",
            description="Aerospace titanium chassis, sapphire crystal display with 14-day battery life, GPS multi-band navigation and real-time ECG health metrics.",
            brand="Quantum",
            base_price=199.99,
            status=ProductStatus.PUBLISHED,
            is_featured=True,
        ...
        v5_black = ProductVariant(product_id=p5.id, sku="WATCH-TITAN-BLK", title="Space Black / 44mm", price=199.99, stock_quantity=35, attributes_json=json.dumps({"color": "Space Black", "size": "44mm"}), is_active=True)
        v5_silver = ProductVariant(product_id=p5.id, sku="WATCH-TITAN-SLV", title="Brushed Silver / 44mm", price=219.99, stock_quantity=25, attributes_json=json.dumps({"color": "Brushed Silver", "size": "44mm"}), is_active=True)
        db.add_all([v5_black, v5_silver])
        await db.flush()
        db.add(ProductImage(product_id=p5.id, image_url="https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=800", is_primary=True, display_order=1))

        # Product 6: CircuitKey Pro Wireless Mechanical Keyboard
        p6 = Product(
            vendor_id=tech_vendor.id,
            category_id=cat_electronics.id,
            title="CircuitKey Pro Tri-Mode Mechanical Keyboard",
            slug="circuitkey-pro-tri-mode-mechanical-keyboard",
            description="Hot-swappable tactile switches, gasket mounted CNC aluminum case, dynamic RGB backlighting and seamless multi-device Bluetooth switching.",
            brand="CircuitKey",
            base_price=149.00,
            status=ProductStatus.PUBLISHED,
            is_featured=True,
        ...
        v6_red = ProductVariant(product_id=p6.id, sku="KB-TRI-RED", title="Red Linear Switches / 75%", price=149.00, stock_quantity=40, attributes_json=json.dumps({"switch": "Red Linear"}), is_active=True)
        v6_brown = ProductVariant(product_id=p6.id, sku="KB-TRI-BRN", title="Brown Tactile Switches / 75%", price=149.00, stock_quantity=30, attributes_json=json.dumps({"switch": "Brown Tactile"}), is_active=True)
        db.add_all([v6_red, v6_brown])
        await db.flush()
        db.add(ProductImage(product_id=p6.id, image_url="https://images.unsplash.com/photo-1587829741301-dc798b83add3?w=800", is_primary=True, display_order=1))

        # Product 7: Nordic ErgoDesk Oak Workstation Chair
        p7 = Product(
            vendor_id=fashion_vendor.id,
            category_id=cat_home.id,
            title="Nordic Studio Ergonomic Linen Armchair",
            slug="nordic-studio-ergonomic-linen-armchair",
            description="Sculptural Scandinavian design crafted with solid white oak legs, high-resilience memory foam cushioning and breathable organic linen upholstery.",
            brand="Nordic Living",
            base_price=389.00,
            status=ProductStatus.PUBLISHED,
            is_featured=True,
        ...
        v7_sand = ProductVariant(product_id=p7.id, sku="CHAIR-OAK-SND", title="Desert Sand Linen", price=389.00, stock_quantity=12, attributes_json=json.dumps({"color": "Desert Sand"}), is_active=True)
        v7_charcoal = ProductVariant(product_id=p7.id, sku="CHAIR-OAK-CHR", title="Charcoal Grey Linen", price=389.00, stock_quantity=15, attributes_json=json.dumps({"color": "Charcoal Grey"}), is_active=True)
        db.add_all([v7_sand, v7_charcoal])
        await db.flush()
        db.add(ProductImage(product_id=p7.id, image_url="https://images.unsplash.com/photo-1580481077111-9a70f2f354f3?w=800", is_primary=True, display_order=1))

        # Product 8: Artisan Leather Commuter Backpack
        p8 = Product(
            vendor_id=fashion_vendor.id,
            category_id=sub_mens.id,
            title="Artisan Full-Grain Leather Commuter Backpack",
            slug="artisan-full-grain-leather-commuter-backpack",
            description="Water-resistant full-grain vegetable tanned leather with dedicated padded 16-inch laptop compartment and solid brass YKK zippers.",
            brand="UrbanStyle",
            base_price=175.00,
            status=ProductStatus.PUBLISHED,
            is_featured=False,
        ...
        v8_cognac = ProductVariant(product_id=p8.id, sku="BP-LTH-COG", title="Vintage Cognac Brown", price=175.00, stock_quantity=20, attributes_json=json.dumps({"color": "Cognac Brown"}), is_active=True)
        v8_black = ProductVariant(product_id=p8.id, sku="BP-LTH-BLK", title="Matte Obsidian Black", price=175.00, stock_quantity=25, attributes_json=json.dumps({"color": "Obsidian Black"}), is_active=True)
        db.add_all([v8_cognac, v8_black])
        await db.flush()
        db.add(ProductImage(product_id=p8.id, image_url="https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=800", is_primary=True, display_order=1))

        # Product 9: Lumina Minimalist LED Desk Lamp
        p9 = Product(
            vendor_id=fashion_vendor.id,
            category_id=cat_home.id,
            title="Lumina Minimalist Dimmable LED Studio Lamp",
            slug="lumina-minimalist-dimmable-led-studio-lamp",
            description="Ultra-slim architectural matte aluminum desk lamp with capacitive touch slider, 5 color temperature presets, and integrated wireless charging pad.",
            brand="Nordic Living",
            base_price=79.00,
            status=ProductStatus.PUBLISHED,
            is_featured=False,
        ...
        v9_white = ProductVariant(product_id=p9.id, sku="LAMP-MAT-WHT", title="Matte Glacier White", price=79.00, stock_quantity=50, attributes_json=json.dumps({"color": "Glacier White"}), is_active=True)
        db.add(v9_white)
        await db.flush()
        db.add(ProductImage(product_id=p9.id, image_url="https://images.unsplash.com/photo-1507473885765-e6ed057f782c?w=800", is_primary=True, display_order=1))

        # Product 10: PureBrew Precision Italian Espresso Machine
        p10 = Product(
            vendor_id=tech_vendor.id,
            category_id=cat_home.id,
            title="PureBrew 15-Bar Precision Thermal Espresso Machine",
            slug="purebrew-15-bar-precision-thermal-espresso-machine",
            description="Commercial-grade 58mm stainless steel portafilter with PID temperature control, dual thermoblocks, and powerful microfoam steam wand.",
            brand="AeroSound",
            base_price=450.00,
            status=ProductStatus.PUBLISHED,
            is_featured=True,
        ...
        v10_steel = ProductVariant(product_id=p10.id, sku="ESP-SS-PRO", title="Brushed Stainless Steel", price=450.00, stock_quantity=15, attributes_json=json.dumps({"color": "Stainless Steel"}), is_active=True)
        db.add(v10_steel)
        await db.flush()
        db.add(ProductImage(product_id=p10.id, image_url="https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?w=800", is_primary=True, display_order=1))
